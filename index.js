...
function randomWord() {
  const randomInd = Math.ceil(Math.random() * wordCount);
...
document.getElementById("game").addEventListener("keyup", (ev) => {
  ...
  if (!window.timer && isLetter) {
    window.timer = setInterval(() => {
      if (!window.gameStart) {
        window.gameStart = pressTime;
      }
      const currentTime = new Date().getTime();
      const timepass = currentTime - window.gameStart;
      const secpass = Math.round(timepass / 1000);
      const secleft = gameTime / 1000 - secpass;
      if (secleft <= 0) {
        gameOver();
        return;
      }
  ...
  if (isBackspace) {
    // Handle extra incorrect letters

    if (currentLetter && currentLetter.classList.contains("extra")) {
      currentLetter.remove();
      currentLetter = currentword.lastChild;
